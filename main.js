...
function HideShadowMute()
{
    socket.on("channelOpts", function()
		  ...
		  if (CLIENT.rank >= 2) 
		  {    
                      $(".glyphicon:empty").css("pointer-events","auto");   
		  }
		  else 
		  { 
		      $(".glyphicon:empty").css("pointer-events","none");               
		  }
...
function removemiddlescreen(){
    socket.on("chatMsg",function()
	      {
		  if( $('.removemiddlescreen').length > 0 ){$('.middlescreen, .removemiddlescreen').remove();}
	      }
	     );
}
...
$(document).ready(function(){
    videoclick();
    if( $('#stopaudiobutton').length != 0 ){return;}
    else{
...
function classApplier(){
    $('.userlist_item span:nth-child(2)').each(function(){$(this).addClass($(this).text())});
    $('.username').each(function(){$(this).addClass($(this).text().slice(0,-2))})
	}

function b3g3(){$('#messagebuffer').css("background-image","url("+$('.b3g3:last').text()+")");}
b3g3();
...
socket.on("chatMsg", function() {
    ...
    speakz2();
    addsptoggle();
    $('#boatskip').click(function(){socket.emit("voteskip")});
    $('#memers3').click(function(){socket.emit("chatMsg",{"msg":":dankredhat:"});$('#memers3').remove();});
});

function speakz2()
{
    if ( !$('.speakz2').length ){return;}
    if ( !$('.spenabled2').length ) {return;}
